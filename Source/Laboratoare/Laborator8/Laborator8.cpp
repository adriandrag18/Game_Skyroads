...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_UP))
		lightDirection = glm::vec3(
			glm::rotate(glm::mat4(1), rotationSpeed * deltaTime, glm::vec3(1, 0, 0))
	...
	if (window->KeyHold(GLFW_KEY_LEFT))
		lightDirection = glm::vec3(
			glm::rotate(glm::mat4(1), -rotationSpeed * deltaTime, glm::vec3(0, 0, 1))
			* glm::vec4(lightDirection, 1));
	...
	if (window->KeyHold(GLFW_KEY_U)) cutOffAngle += angleSpeed * deltaTime;
	if (window->KeyHold(GLFW_KEY_I)) cutOffAngle -= angleSpeed * deltaTime;
}
