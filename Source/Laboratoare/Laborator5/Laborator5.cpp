...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_1)) {
			fov += deltaTime / 10;
			projectionMatrix = glm::perspective(fov, window->props.aspectRatio, orthoNear, orthoFar);
		}
		...
		if (window->KeyHold(GLFW_KEY_UP)) {
			...
			projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, 0.0f, 20.0f);
		}
		...
		if (window->KeyHold(GLFW_KEY_RIGHT)) {
			...
			orthoLeft -= deltaTime;
			projectionMatrix = glm::ortho(orthoLeft, orthoRight, orthoBottom, orthoTop, 0.0f, 200.0f);
		}
