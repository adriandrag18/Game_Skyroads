...
void Laborator9::Init()
{
...

	{
		...
		for (float h = 0; h < 2; h++)
		{
			...
			{
				vertices.push_back(glm::vec3(cos(angle), 50 * h, sin(angle)));
		...
		vertices.push_back(glm::vec3(0, 30, 15));
		normals.push_back(glm::vec3(0, 1, 0));
		...
		vertices.push_back(glm::vec3(30, 40, -15));
		normals.push_back(glm::vec3(0, 0, 0));
		...
		points = vertices.size();
		vertices.push_back(glm::vec3(0, 50, 15));
		normals.push_back(glm::vec3(0, 1, 0));
		textureCoords.push_back(glm::vec2(1, 1));
		...
		vertices.push_back(glm::vec3(30, 60, -15));
		normals.push_back(glm::vec3(0, 0, 0));
		textureCoords.push_back(glm::vec2(0, 0.2));
...
void Laborator9::Update(float deltaTimeSeconds)
{
	...
	isGlobe = 0;
	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["crate"], mapTextures["grass"]);
	}
...

	{
		...
		RenderSimpleMesh(meshes["bamboo2"], shaders["ShaderLab9"], modelMatrix, mapTextures["bamboo"]);
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	int to_mix = glGetUniformLocation(shader->program, "to_mix");
	glUniform1i(to_mix, mix);
