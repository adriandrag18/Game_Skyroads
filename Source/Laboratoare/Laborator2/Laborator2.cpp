...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vector<VertexFormat> vertex_tetrahedron
		{
			VertexFormat(A, red),
			VertexFormat(C, red),
			VertexFormat(B, red),

			VertexFormat(C, green),
			VertexFormat(B, green),
			VertexFormat(D, green),

			VertexFormat(D, blue),
			VertexFormat(B, blue),
			VertexFormat(A, blue),

			VertexFormat(A, yellow),
			VertexFormat(B, yellow),
			VertexFormat(D, yellow),
		};
		...
		vector<VertexFormat> vertex_square
		{
			VertexFormat(glm::vec3(1, 1, 0), red),
			VertexFormat(glm::vec3(1, 0, 0), red),
			VertexFormat(glm::vec3(0, 1, 0), red),
			VertexFormat(glm::vec3(0, 0, 0), red),
		};
		...
		meshes["square"]->InitFromData(vertex_square, indices_square);

		
		meshes["tetrahedron"] = new Mesh("generated tetrahedron");
		meshes["tetrahedron"]->InitFromData(vertex_tetrahedron, indices_tetrahedron);
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(1, 0, 0), glm::vec3(0.25f));

	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(1, 1, 1), glm::vec3(0.25f));
	
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(2, 0, 2));
